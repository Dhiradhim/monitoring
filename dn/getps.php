...
include("../koneksi.php");
$id = $_GET['id'];
$q_ps="SELECT kd_ps, sn_ps, pn_ps FROM tb_ps";
$ps = mysqli_query($con, $q_ps);
$row_ps = mysqli_fetch_assoc($ps);?>
			...
			<label class="control-label col-md-3 col-sm-3 col-xs-12" for="namabarang<?=$id?>">Nama Barang <?=$id?> <span class="required">*</span>
				...
				<option value="-">-</option>
                            <?php
                                ...
                                do {?>
                                <option value="<?php echo $row_ps['kd_ps'];?>"><?php echo $row_ps['kd_ps'];?> (SN : <?php echo $row_ps['sn_ps'];?> | PN : <?=$row_ps['pn_ps']?>)</option>
                                <?php
